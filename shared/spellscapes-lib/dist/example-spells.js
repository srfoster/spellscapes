...
exports.createBlindWanderingGrazer = void 0;
...
class FourWayMovementActuator {
    ...
    getIntentions() {
        if (this.goNorth)
            return [new spells_1.SquareRelative("move", -1, 0)];
        if (this.goSouth)
            return [new spells_1.SquareRelative("move", 1, 0)];
        if (this.goEast)
            return [new spells_1.SquareRelative("move", 0, -1)];
        if (this.goWest)
            return [new spells_1.SquareRelative("move", 0, 1)];
    }
}
function createBlindWanderingGrazer() {
    return new spells_1.Spell({ components: { sensor_cores: [],
            program_cores: [new WanderRandomly()],
            actuator_cores: [new FourWayMovementActuator()] }
    });
}
exports.createBlindWanderingGrazer = createBlindWanderingGrazer;
